...
export default function TransferFlowModal({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Auto-select: prefer same weight class, fallback to wildcard
      const weightMatch = enriched.find(
        (s) => s.slot_type === addFighter!.weight_class
      );
        ...
        {loading ? (
          ...
          <>
            <div className="flex items-center space-x-4 bg-emerald-900/10 border-2 border-emerald-800/40 rounded-xl p-3 mb-5">
              <div className="relative w-12 h-12 rounded-full bg-zinc-900 border-[3px] border-emerald-500/70 flex-shrink-0 overflow-visible">
                {addFighter.image_url && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={addFighter.image_url} alt={addFighter.name} className="w-full h-full rounded-full object-cover" />
                )}
              </div>
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Adding</span>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <div className="relative w-10 h-10 rounded-full bg-zinc-900 border-[3px] border-zinc-700 flex-shrink-0">
